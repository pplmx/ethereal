...
mod tests {
    ...
    fn test_overheating_priority() {
        ...
        let state = determine_state(&monitor, AppCategory::Idle, &config);
    ...
    fn test_coding_activity() {
        ...
        let config = create_config(80.0);
        let state = determine_state(&monitor, AppCategory::Coding, &config);
    ...
    fn test_gaming_activity() {
        let monitor = MockMonitor { temp: 60.0, util: 10.0 };
        let config = create_config(80.0);
        let state = determine_state(&monitor, AppCategory::Gaming, &config);
    ...
    fn test_idle_default() {
        let monitor = MockMonitor { temp: 60.0, util: 10.0 };
        let config = create_config(80.0);
        let state = determine_state(&monitor, AppCategory::Unknown, &config);
